...
# Various inputs and directories for the member profiles

server_name <- "AWS-PROD-SQL\\COHO"
database_name <- "Elmer"

# Community / Places Shapefile
...
# Information need to clean table creation - Travel Time
tt_var <- "B08303"
...
# Information need to clean table creation - Mode Share
...
# Information need to clean table creation - Occupations
occ_var <- "OCCUPATION"
...
# Information need to clean table creation - Industry
ind_var <- "INDUSTRY"
...
# Information need to clean table creation - Income
...
inc_remove <- NULL
inc_order <- c("less than 10k","10k to 15k","15k to 25k","25k to 35k","35k to 50k","50k to 75k","75k to 100k","100k to 150k","150k to 200k","more than 200k")

# Functions for Member Profiles
...
find_city_county <- function(wrk_nm) {
  wrk_cnty <- as.character(community.point[NAME == wrk_nm,COUNTY])
  return(wrk_cnty)
}
...
create_mode_map <- function(w_tbl, w_yr, w_color, w_mode, w_place, w_group, w_title) {
  ...
  working_map <- leaflet(data = current_value) %>% 
    ...
    addLegend("bottomright", pal=pal, values = current_value$estimate_percent,
              ...
              opacity = 1) %>%
    setView(lng=find_city_coordinates(w_place,"INTPTLON"), lat=find_city_coordinates(w_place,"INTPTLAT"), zoom=find_city_coordinates(w_place,"ZOOM"))
...
# Various items to cleanup in tables from database for clean presentation - should get incorporated into DB
...
data_places <- unique(wa_places$place_name)
...
output$wrk_map <- renderLeaflet({
  leaflet() %>%
    ...
    addPolygons(data = community.shape[which(community.shape$NAME %in% input$Place),],
                ...
                dashArray = "4",
                fillOpacity = 0.0)%>%
    setView(lng=find_city_coordinates(input$Place,"INTPTLON"), lat=find_city_coordinates(input$Place,"INTPTLAT"), zoom=find_city_coordinates(input$Place,"ZOOM"))
})
...
# Jobs
numeric_ind <- c("estimate","margin_of_error")
percent_ind <- c("Share","Region")
job_length <- 10

output$table_occ <- DT::renderDataTable({
  datatable(create_summary_table(wa_places,input$Place,input$Year,"variable_category",occ_var,occ_cols,occ_total,occ_remove,occ_order),rownames = FALSE, options = list(pageLength = job_length, columnDefs = list(list(className = 'dt-center', targets =1:4)))) %>% formatCurrency(numeric_ind, "", digits = 0) %>% formatPercentage(percent_ind, 1)
})

output$table_ind <- DT::renderDataTable({
  datatable(create_summary_table(wa_places,input$Place,input$Year,"variable_category",ind_var,ind_cols,ind_total,ind_remove,ind_order),rownames = FALSE, options = list(pageLength = job_length, columnDefs = list(list(className = 'dt-center', targets =1:4)))) %>% formatCurrency(numeric_ind, "", digits = 0) %>% formatPercentage(percent_ind, 1)
})

output$table_inc <- DT::renderDataTable({
  datatable(create_summary_table(wa_places,input$Place,input$Year,"variable_name",inc_var,inc_cols,inc_total,inc_remove,inc_order),rownames = FALSE, options = list(pageLength = job_length, columnDefs = list(list(className = 'dt-center', targets =1:4)))) %>% formatCurrency(numeric_ind, "", digits = 0) %>% formatPercentage(percent_ind, 1)
})
...
mainPanel(
      ...
      tabsetPanel(type = "tabs",
                  ...
                  tabPanel("TIP", leafletOutput("tip_map", width = "100%" )),
                  tabPanel("RTP", leafletOutput("rtp_map", width = "100%" ))
      ),
width = 12
)
